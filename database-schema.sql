-- =============================================
-- RETELL Database Schema - Production Ready
-- =============================================
-- This script addresses all previous issues:
-- 1. Proper RLS policies with INSERT permissions
-- 2. Automatic user profile creation via triggers
...
-- User Profiles Policies (FIXED - includes INSERT policy)
...
-- This solves the RLS timing issue by using security definer
...
CREATE OR REPLACE TRIGGER update_podcasts_updated_at BEFORE UPDATE ON podcasts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_episodes_updated_at BEFORE UPDATE ON episodes FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_licenses_updated_at BEFORE UPDATE ON licenses FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE OR REPLACE TRIGGER update_questionnaire_templates_updated_at BEFORE UPDATE ON questionnaire_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_questionnaire_responses_updated_at BEFORE UPDATE ON questionnaire_responses FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE OR REPLACE TRIGGER update_contracts_updated_at BEFORE UPDATE ON contracts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
INSERT INTO questionnaire_templates (name, description, schema, is_active) VALUES 
('Default Licensing Questionnaire', 'Standard licensing questionnaire for podcast content', '{
  "fields": [
    {
      "id": "content_type",
      "type": "select",
...
-- Insert sample agencies data
INSERT INTO agencies (name, description, contact_email, country, website) VALUES 
...
-- This database is now ready for production use with:
-- ✅ Proper RLS policies with INSERT permissions
-- ✅ Automatic user profile creation via triggers
-- ✅ Security-focused design (no public admin signup)
